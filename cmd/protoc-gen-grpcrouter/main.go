...
func generateHelperMethods(g *protogen.GeneratedFile, serviceName, implName string, routerService *protogen.Service) error {
	...
	g.P("// This method handles real-time message forwarding from service instances")
	g.P("// It processes RPC responses and forwards them immediately to client streams")
	g.P()
	g.P("// TODO: In full implementation, this would read from the registration stream")
	g.P("// and forward messages in real-time based on request ID correlation")
	g.P()
	g.P("// Placeholder implementation for now")
	g.P("log.Printf(\"Starting message handling for instance %s\", instance.InstanceID)")
	g.P()
	g.P("// Real implementation would look like:")
	g.P("// for {")
	g.P("//   regReq, err := instance.registrationStream.Recv()")
	g.P("//   if err != nil { return err }")
	g.P("//")
	g.P("//   if rpcResp := regReq.GetRpcResponse(); rpcResp != nil {")
	g.P("//     // Forward immediately to correlated client stream")
	g.P("//     if clientStream := r.correlator.GetStreamForRequestId(rpcResp.RequestId); clientStream != nil {")
	g.P("//       // Real-time relay - no buffering")
	g.P("//       r.forwardResponseToClientStream(rpcResp, clientStream)")
	g.P("//     }")
	g.P("//   }")
	g.P("// }")
	g.P()
	g.P("// For now, just return - this gets called but doesn't block registration")
	g.P("return nil")
	g.P("}")
	g.P()
	...
	g.P("func (r *", implName, ") handleServerStreamingCall(instance *router.ServiceInstance, call *", serviceName, "RpcCall, clientStream interface{}) error {")
	g.P("// Send RPC call immediately to service instance through registration stream")
	g.P("r.instancesMux.RLock()")
	g.P("registeredInstance, exists := r.instances[instance.InstanceID]")
	g.P("r.instancesMux.RUnlock()")
	g.P()
	g.P("if !exists {")
	g.P(`return status.Errorf(codes.Unavailable, "service instance %s not found", instance.InstanceID)`)
	g.P("}")
	g.P()
	g.P("// Set up streaming correlation for real-time response forwarding")
	g.P("// TODO: Implement proper streaming correlation when correlator supports it")
	g.P("log.Printf(\"Setting up streaming correlation for request %s\", call.RequestId)")
	g.P("_ = clientStream // Avoid unused warning for now")
	g.P()
	g.P("// Send call to service bridge immediately (no waiting)")
	g.P("regResponse := &", serviceName, "RegisterResponse{")
	g.P("Response: &", serviceName, "RegisterResponse_RpcCall{")
	g.P("RpcCall: call,")
	g.P("},")
	g.P("}")
	g.P()
	g.P("// TODO: Send through actual registration stream to service instance")
	g.P("// For now, simulate the streaming call")
	g.P("log.Printf(\"Initiated server streaming call %s to instance %s\", call.RequestId, instance.InstanceID)")
	g.P()
	g.P("// In real implementation, this would block until stream completion")
	g.P("// Stream responses are forwarded in real-time via handleInstanceMessages")
	g.P("_ = registeredInstance // Avoid unused warning")
	g.P("_ = regResponse // Avoid unused warning")
	g.P()
	g.P("// For now, return success immediately")
	g.P("return nil")
	g.P("}")
	g.P()
...
func generateMethodHandler(g *protogen.GeneratedFile, method *protogen.Method, serviceName, bridgeName string) error {
	...
	// Handle streaming methods with real-time implementation
	if method.Desc.IsStreamingClient() || method.Desc.IsStreamingServer() {
		...
		if method.Desc.IsStreamingServer() {
			// Server streaming - implement real-time response forwarding
			g.P("// Server streaming: call service immediately and forward responses in real-time")
			g.P("ctx := context.Background()")
			g.P("serviceStream, err := b.service.", methodName, "(ctx, req)")
			g.P("if err != nil {")
			g.P(`log.Printf("[%s] Failed to start `, methodName, ` stream: %v", b.options.InstanceID, err)`)
			g.P("return &", serviceName, "RpcResponse{")
			g.P("RequestId: requestID,")
			g.P("Status: &pb.RpcStatus{Code: 13, Message: fmt.Sprintf(\"Stream start failed: %v\", err)},")
			g.P("Metadata: make(map[string]string),")
			g.P("}")
			g.P("}")
			g.P()
			g.P("// Forward each response immediately as it arrives (real-time relay)")
			g.P("go func() {")
			g.P("defer func() {")
			g.P("if r := recover(); r != nil {")
			g.P(`log.Printf("[%s] Panic in `, methodName, ` streaming: %v", b.options.InstanceID, r)`)
			g.P("}")
			g.P("}()")
			g.P()
			g.P("for {")
			g.P("resp, err := serviceStream.Recv()")
			g.P("if err == io.EOF {")
			g.P("// End of stream - send completion signal")
			g.P("b.sendResponse(&", serviceName, "RpcResponse{")
			g.P("RequestId: requestID,")
			g.P("Status: &pb.RpcStatus{Code: 0, Message: \"EOF\"},")
			g.P("Metadata: make(map[string]string),")
			g.P("})")
			g.P("return")
			g.P("}")
			g.P("if err != nil {")
			g.P("// Stream error - send error signal")
			g.P("b.sendResponse(&", serviceName, "RpcResponse{")
			g.P("RequestId: requestID,")
			g.P("Status: &pb.RpcStatus{Code: 13, Message: fmt.Sprintf(\"Stream error: %v\", err)},")
			g.P("Metadata: make(map[string]string),")
			g.P("})")
			g.P("return")
			g.P("}")
			g.P()
			g.P("// Send response immediately (real-time relay)")
			g.P("b.sendResponse(&", serviceName, "RpcResponse{")
			g.P("RequestId: requestID,")
			g.P("Status: &pb.RpcStatus{Code: 0, Message: \"Streaming\"},")
			g.P("Metadata: make(map[string]string),")
			...
			// Client or bidirectional streaming - placeholder for now
			g.P("// Client/bidirectional streaming - placeholder implementation")
			g.P("_ = req // Avoid unused variable warning")
		}
		...
		g.P("Status: &pb.RpcStatus{")
		g.P("Code:    0, // OK")
		g.P(`Message: "Streaming initiated",`)
		g.P("},")
		g.P("Metadata: make(map[string]string),")
		g.P("}")
		g.P("}")
